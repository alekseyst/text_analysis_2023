...
* Чтобы слова с дефисами считались одним токеном, выберите Connector и Dash в меню File — Open Corpus Manager — Raw File(s) — Basic Settings — Show Token Definition Settings — Unicode Punctuation. Если нужно, там же (Open Corpus Manager — Raw File(s)) настройте кодировку.
* Работая с файлами, в которых приводятся леммы и части речи (лемма_POS), выберите там же настройку Indexer — simple_pos_headword_indexer
